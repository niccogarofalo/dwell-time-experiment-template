...
- **How To: Display A Specific Slideshow**  
  To show a specific slideshow, you need to use a 'URL Parameter' named "slideshow". The value of the parameter is the name of the slideshow you would like to show during the experiment.
  See below for example:  
  - **How to: Add a URL parameter**  
    Suppose you have the URL example.com/website  
    To add any URL parameter(s), you must first add a question mark (?) to the end of the URL  
    Then, you add the name of the parameter, an equals sign, and finally the value of the parameter.   
    Thus, if you wanted to add a URL parameter titled 'name' and assign it the value 'megan' the URL would be:  
    example.com/website?name=megan  
    ...
    example.com/website?name=megan&lastName=rapinoe
  - This practice can be extended to our experiment to determine which slideshow to play.  
    Your trial URL should have the form: run.pavlovia.org/[my-account-name]/[my-experiment-name]...  
    ...
    Remember, if the URL already has parameters (such as the URLs in the 'Piloting' mode), add an (&) instead of a (?).  
...
- Once all these steps have been completed, you should now be able to test your new slideshow by using the 'slideshow' URL parameter with the official name of the slideshow!
